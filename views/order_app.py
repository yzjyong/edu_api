...
@order_blue.route("/comfirmorder/",methods=["GET",])
def comfirm_order():
    ...
    if u_id:
        if type1 == "fromcart":  #如果是从购物车提交订单
                ...
                if total_price==total:   #提交订单传递的总价是否和后台计算的总价相等
                    ...
                    for course in course_info:  #创建订单记录
                        order_success = order_obj.save("orders",**{"order_num":order_num,"user_id":u_id,
                                                    "course_id":course.get("id"),"price":course.get("price") })
                        if order_success:  #如果创建订单成功，删除购物车记录
            ...
            if total_price == total:   #总价相等，创建一条订单记录
                ...
                order_num = crypt.get_order_code()
                order_success = order_obj.save("orders",**{"order_num":order_num,"user_id":u_id,
                                                "course_id":course_info[0].get("cid"),"price":course_info[0].get("price") })
                if order_success:  #如果创建订单记录成功，返回订单信息
                    return jsonify({"code":200,"order_num":order_num,"course":course_info})
            return jsonify({"code": 201, "msg": "价格不正确"})
...
@order_blue.route("/myorder/",methods=["GET",])
def my_order():
...
if __name__ == '__main__':
    import base64
    print(base64.b16encode("12".encode()))
